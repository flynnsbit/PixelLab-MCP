...
interface PixelLabResponse {
  ...
  template_id?: string;
  view?: string;
  rotation_urls?: { [key: string]: string | null };
  style_settings?: any;
  guidance?: any;
  ai_freedom?: any;
  tags?: any[];
...
class PixelLabServer {
  ...
  private async handleCreateCharacter(args: any) {
    if (!args || typeof args.description !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description is required for character creation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/create-character-with-4-directions', {
  ...
  private async handleAnimateCharacter(args: any) {
    if (!args || !args.character_id) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Character ID is required for animation'
      );
    }
    ...
    try {
      ...
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/characters/animations', {
  ...
  private async handleCreateTopdownTileset(args: any) {
    if (!args || !args.lower || !args.upper) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Lower and upper terrain types are required'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/tilesets', {
  ...
  private async handleCreateSidescrollerTileset(args: any) {
    return {
      content: [
        {
          type: 'text',
  ...
  private async handleCreateIsometricTile(args: any) {
    if (!args || typeof args.description !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description is required for isometric tile creation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/create-isometric-tile', {
        description: args.description,
        image_size: {
          width: args.size || 32,
          height: args.size || 32
  ...
  private async handleGetCharacter(args: any) {
    if (!args || !args.character_id) {
      throw new McpError(
        ErrorCode.InvalidParams,
    ...
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/characters/${args.character_id}`);
      ...
      if (character.rotation_urls && character.rotation_urls.south) {
        ...
        if (character.animations && character.animations.length > 0) {
          ...
          character.animations.forEach((animation: any, index: number) => {
  ...
  private async handleListCharacters(args: any) {
    try {
      const params = new URLSearchParams();
      if (args?.limit) params.append('limit', args.limit.toString());
      if (args?.offset) params.append('offset', args.offset.toString());

      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/characters?${params.toString()}`);
  ...
  private async handleGetBackgroundJob(args: any) {
    if (!args || !args.job_id) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Job ID is required'
    ...
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/background-jobs/${args.job_id}`);
  ...
  private async handleGetBalance(args: any) {
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get('/balance');
  ...
  private async handleGet8DirectionCharacter(args: any) {
    if (!args || typeof args.description !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description is required for 8-direction character creation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/create-character-with-8-directions', {
        description: args.description,
        image_size: {
          width: 64,
          height: 64
  ...
  private async handleGetCharacterZip(args: any) {
    if (!args || !args.character_id) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Character ID is required for ZIP download'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/characters/${args.character_id}/zip`);
  ...
  private async handleGetTilesetStatus(args: any) {
    ...
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/tilesets/${args.tileset_id}`);
  ...
  private async handleGetIsometricTileStatus(args: any) {
    ...
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.get(`/isometric-tiles/${args.tile_id}`);
  ...
  private async handleAnimateWithText(args: any) {
    if (!args || typeof args.description !== 'string' || typeof args.action !== 'string' || typeof args.reference_image !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description, action, and reference_image (character ID) are required for text-based animation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/animate-with-text', {
        image_size: {
          width: 64,
          height: 64
        },
  ...
  private async handleAnimateCharacterAlt(args: any) {
    if (!args || !args.character_id) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Character ID is required for animation'
    ...
    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/animate-character', {
        character_id: args.character_id,
  ...
  private async handleCreateImagePixflux(args: any) {
    if (!args || typeof args.description !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description is required for image generation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/create-image-pixflux', {
        description: args.description,
  ...
  private async handleInpaintPixelArt(args: any) {
    if (!args || typeof args.description !== 'string' || typeof args.source_image !== 'string' || typeof args.mask_image !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description, source_image, and mask_image are required for inpainting'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/inpaint', {
        description: args.description,
        negative_description: args.negative_description,
        image_size: {
          width: args.width || 64,
  ...
  private async handleCreateImageBitforge(args: any) {
    if (!args || typeof args.description !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Description is required for Bitforge image generation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/create-image-bitforge', {
        description: args.description,
        negative_description: args.negative_description,
        image_size: {
          width: args.width || 64,
  ...
  private async handleRotateCharacter(args: any) {
    if (!args || typeof args.source_image !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'source_image (base64) is required for character rotation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/rotate', {
        from_image: {
          type: 'base64',
          base64: args.source_image,
          format: 'png'
  ...
  private async handleAnimateWithSkeleton(args: any) {
    if (!args || typeof args.reference_image !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'reference_image (base64) is required for skeleton animation'
      );
    }
    ...
    try {
      ...
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/animate-with-skeleton', requestBody);
  ...
  private async handleEstimateSkeleton(args: any) {
    if (!args || typeof args.image !== 'string') {
      throw new McpError(
        ErrorCode.InvalidParams,
        'image (base64) is required for skeleton estimation'
      );
    }

    try {
      const response: AxiosResponse<PixelLabResponse> = await this.axiosInstance.post('/estimate-skeleton', {
        image: {
          type: 'base64',
          base64: args.image,
          format: 'png'
      ...
      if (response.status === 200) {
        ...
        if (keypoints.length > 0) {
          ...
          keypoints.forEach((point: number[], index: number) => {
  ...
  private async handleGetApiDocumentation(args: any) {
    try {
      const response: AxiosResponse<string> = await this.axiosInstance.get('/llms.txt');
